...
class CreateReview extends Component {
  ...
  renderField(field)
  {
    ...
    <div className={className}>
          ...
          <input className={field.label === "Express ya self!" ? "form-control reviewbodyForm" : "form-control"}
          {...field.input} placeholder={field.placeholder} type={field.type}
          />
          </div>
  ...
  onDelete(){
    ...
    this.props.deleteReview(bootname, reviewID, () => this.props.fetchReviews(bootbrand, bootname));
    this.props.reviewDone();
  }
  ...
  onSubmit(values) {
    ...
    if(!this.props.alreadyHaveValues)
    this.props.submitReview(values, () => {this.props.fetchReviews(bootbrand, bootname)});
    else
    {
      values._reviewID = reviewID;
      this.props.editReview(values, () => {this.props.fetchReviews(bootbrand, bootname)});
    }
  ...
  render() {
    ...
    return(<div className="editingContainer editingReview robot">
          ...
          <button type="submit" className="btn payment-btn text-white btn-space btn-space-top" style={{backgroundColor: '#8A2BE2'}}> Submit </button>
              ...
              return(<div>Loading Modules...</div>);
                  ...
                  return(<div className="editingContainer editingReview robot">
                      ...
                      <div className="rightRating">
                        ...
                        <button type="button" onClick={() => {this.onDelete()}} className="btn btn-danger text-white btn-space btn-space-top">Delete</button>
                        <button type="submit" className="btn payment-btn text-white btn-space btn-space-top" style={{backgroundColor: '#8A2BE2'}}> Submit Edits</button>
                        <button type="button" onClick={this.props.reviewDone} className="btn btn-secondary payment-btn text-white btn-space-top"> Cancel </button>
                        </form>
                        </div>);
              }
...
function validate(values)
{
  ...
  if(!values.ratingOverall)
    errors.ratingOverall = "Need a overall rating";
  else{
  ...
  if(!values.ratingDurability)
    errors.ratingDurability = "Need a durability rating";
  else{
  ...
  if(!values.ratingTouch)
     errors.ratingTouch = "Need a touch rating";
  else{
  ...
  if(!values.ratingTraction)
     errors.ratingTraction = "Need a traction rating";
  else{
  ...
  if(!values.ratingProtection)
     errors.ratingProtection = "Need a protections rating";
  else{
...
CreateReview = onClickOutside(CreateReview);
      ...
      export default reduxForm({
        ...
        connect(mapStateToProps, {deleteReview, editReview, submitReview, fetchReviews, fetchReview })(CreateReview)
      );
